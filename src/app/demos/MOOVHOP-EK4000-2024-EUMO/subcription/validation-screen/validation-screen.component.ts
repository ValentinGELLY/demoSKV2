  ...
  override ngOnInit(): void {
    ...
    if (this.scanVisited === 3 && this.identityValidate) {
      ...
      this.timeout = setTimeout(() => {
        ...
        this.route.navigate(['/EUMO2024/createAccountFaceCapture']);
      }, 5000);
    }else if (this.scanVisited === 2 && !this.moovhopService.errorSaveIdCard) {
      this.timeout = setTimeout(() => {
        this.route.navigate(['/EUMO2024/createAccountPersonalInformations']);
      }, 5000);
    }else if (this.scanVisited === 3 && !this.identityValidate) {
      this.timeout = setTimeout(() => {
        this.route.navigate(['/EUMO2024/createAccountMenu']);
      }, 5000);
