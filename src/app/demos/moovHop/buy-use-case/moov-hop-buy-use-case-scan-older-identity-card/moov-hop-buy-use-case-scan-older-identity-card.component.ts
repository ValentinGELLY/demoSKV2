...
@Component({
  selector: 'app-moov-hop-buy-use-case-scan-older-identity-card',
  templateUrl: './moov-hop-buy-use-case-scan-older-identity-card.component.html',
  styleUrls: ['./moov-hop-buy-use-case-scan-older-identity-card.component.scss', '../../moovHop.scss']
})
export class MoovHopBuyUseCaseScanOlderIdentityCardComponent extends GenericComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  override ngOnInit(): void {
    let _this = this;
    _this.skService.addEventApplication("demoSKV2", "début de vie du composant MoovHopBuyUseCaseScanOlderIdentityCardComponent");
  }
  ...
  override onImageDocumentCapture = (e: any) => {
    switch (e.data.dataType) {
      case 'ImageCaptured':
        ...
        setTimeout(() => {
          this.moovHopService.imageCapture = this.skService.lastCaptureImageRaw();
        }, 500);
        // traitement pour le changement de vue
        let navEvent = new CustomEvent("moovHopNav", {
          detail: {
            "delay": 2000,
            "goTo": "/moovHopBuyUseCaseOlderCIScanFinished"
          }
        });
  ...
  ngOnDestroy(): void {
    let ___this = this;
    ___this.skService.addEventApplication("demoSKV2", "fin d'écoute de l'event previewStop de la callback onPreview, composant: MoovHopBuyUseCaseScanOlderIdentityCardComponent");
